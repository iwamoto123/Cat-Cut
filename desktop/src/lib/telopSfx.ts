...
/**
 * W13-5: 直前のsfx付きスロットと同じ効果音なら後のスロットでは鳴らさない(アニメは維持)。
 * Remotion(computeSfxEvents)とプレビュー(PreviewPlayer)の両方がこの関数で判定する。
 * 「直前」はsfxが解決されたスロット同士の並びで見る(sfxなしスロットを挟んでもリセットしない)。
 */
export function isRepeatedSfx(prevSlotSfxId: string | null | undefined, sfxId: string): boolean {
...
export function computeSfxEvents(input: ComputeSfxEventsInput): SfxEvent[] {
  ...
  // W13-5: 加えて、直前のsfx付きスロットと同じ効果音は鳴らさない(同種の連続で煩くなるため)。
  // 2つのガードは併存し、どちらかに該当すればスキップする
  const minIntervalFrames = (minIntervalMs / 1000) * input.fps;
  const events: SfxEvent[] = [];
  let lastFrame = Number.NEGATIVE_INFINITY;
  let prevSlotSfxId: string | null = null;
