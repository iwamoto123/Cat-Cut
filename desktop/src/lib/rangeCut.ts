...
import { autoTelopTextFromWords, splitSceneAtMs, type Scene } from "./scenes.ts";
import {
...
export type RangeCutOptions = {
  ...
  snapMs?: number;
  /** カット後の各半分に確保する最小シーン長(ms)。既定200ms。下回る側は縮退する。 */
  ...
  /**
   * 中央範囲を分割せず、範囲内の単語をdeleted化して1シーンのまま維持する。
...
export type RangeCutMode =
  ...
  | "split"
  /** 単一シーン維持: 中央範囲内の単語をdeleted化。 */
  | "deleteWords"
  /** 先頭側縮退: シーン開始を範囲終端へ動かすstartエッジトリム相当。 */
  | "trimStart"
  /** 末尾側縮退: シーン終了を範囲始端へ動かすendエッジトリム相当。 */
  | "trimEnd"
  /** 全域カバー(または両側縮退): 全単語deleted化(スタブ行+復元と同じ状態)。 */
  | "deleteAll";

export type RangeCutResult = {
  /** 更新後のシーン配列(変更が無ければ元の配列をそのまま返す)。 */
  scenes: Scene[];
  /** 実際にカットされた範囲の始端(ms)。縮退時はシーン端まで広がる。UIラベル用。 */
 ...
 *
 * - 通常ケース(範囲の左右両側に単語が残り、かつ両側が最小シーン長を確保できる):
 *   splitSceneAtMs(境界=範囲の中間点)で分割し、前半end=範囲始端・後半start=範囲終端へ
 *   withUpdatedBoundsで独立に更新する(連動ロールは発生させない)。
 * - 先頭側が縮退(範囲がシーン先頭に接する/先頭側に単語が残らない/最小長未満):
 *   分割せずシーン開始を範囲終端へ動かす(startエッジトリム相当)。末尾側も同様。
 * - 両側とも縮退(範囲がシーン全域を覆う等): 全単語deleted化
 *   (手動のシーン丸ごと削除=スタブ行+復元と同じ状態。autoTrimmedは付けない)。
...
export function cutSceneRangeMs(
  ...
  if (
    ...
    // 範囲内に単語が無い(無音だけ)場合は従来の分割へ倒す。keep_segments の穴は
    // シーンを分けないと作れないため、1シーン維持は「切る対象の単語があるとき」に限る。
    if (wordsChanged) {
      const telopText = scene.telopEdited ? scene.telopText : autoTelopTextFromWords(words);
      const next = scenes.slice();
      next[sceneIndex] = { ...scene, words, telopText };
      return { scenes: next, appliedStartMs: startMs, appliedEndMs: endMs, mode: "deleteWords" };
    }
  }

  // 各半分が「シーンとして残せる」条件: その側に中点を持つ単語が存在し、かつ最小シーン長を確保できる。
  const leftKeepsWords = scene.words.some((word) => (word.startMs + word.endMs) / 2 < startMs);
  const rightKeepsWords = scene.words.some((word) => (word.startMs + word.endMs) / 2 >= endMs);
  const leftOk = leftKeepsWords && startMs - scene.sourceStartMs >= minDurationMs;
  const rightOk = rightKeepsWords && scene.sourceEndMs - endMs >= minDurationMs;

  if (leftOk && rightOk) {
    // 境界は範囲の中間点。left/rightKeepsWordsにより境界の両側に単語があるため分割は必ず成立する。
    const boundaryMs = Math.round((startMs + endMs) / 2);
    const splitScenes = splitSceneAtMs(scenes, sceneId, boundaryMs);
    ...
    next[sceneIndex] = withUpdatedBounds(first, first.sourceStartMs, startMs);
    next[sceneIndex + 1] = withUpdatedBounds(second, endMs, second.sourceEndMs);
    return { scenes: next, appliedStartMs: startMs, appliedEndMs: endMs, mode: "split" };
  }

  if (rightOk) {
    // 先頭側縮退: シーン開始〜範囲終端までを丸ごとカットする(startエッジトリム相当)。
    const next = scenes.slice();
    next[sceneIndex] = withUpdatedBounds(scene, endMs, scene.sourceEndMs);
    return { scenes: next, appliedStartMs: scene.sourceStartMs, appliedEndMs: endMs, mode: "trimStart" };
  }

  if (leftOk) {
    // 末尾側縮退: 範囲始端〜シーン終了までを丸ごとカットする(endエッジトリム相当)。
    const next = scenes.slice();
    next[sceneIndex] = withUpdatedBounds(scene, scene.sourceStartMs, startMs);
    return { scenes: next, appliedStartMs: startMs, appliedEndMs: scene.sourceEndMs, mode: "trimEnd" };
  }

  // 全域カバー(または両側縮退): 全単語deleted化。手動のシーン丸ごと削除(スタブ行+復元)と
  // 同じ状態にするため、autoTrimmedは付けない(トリムで自動復活させない)。
  let wordsChanged = false;
  const words = scene.words.map((word) => {
  ...
  if (!wordsChanged) {
    return { scenes, appliedStartMs: scene.sourceStartMs, appliedEndMs: scene.sourceEndMs, mode: "none" };
  }
  const telopText = scene.telopEdited ? scene.telopText : autoTelopTextFromWords(words);
  const next = scenes.slice();
  next[sceneIndex] = { ...scene, words, telopText };
