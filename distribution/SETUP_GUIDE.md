...
1. NextcloudのREADMEに記載されたzip（`CatCut-haifu-YYYYMMDD-HHMMSS.zip`）をダブルクリックで展開する
2. 展開したフォルダを開き、`install.command` を **右クリック →「開く」→「開く」**
   （ダブルクリックだと「開発元を確認できません」と出て開けないため、必ず右クリックから）
3. 黒い画面（ターミナル）の冒頭にある「配布ビルド」が、今回の更新内容に記載された番号と一致することを確認します。Macのパスワードを聞かれたら入力してください
4. 初回は20〜40分かかります。「インストール完了です」と出たら終わりです。Finderに起動ファイルが表示されます
...
- 二重に起動すると、既に開いている編集画面へ戻ります。アプリを完全に終了する場合は、保存済みを確認して⌘Qを押してください
...
同じ日の更新も時刻で区別できます。zipと同じ日時の `CatCut-setup-YYYYMMDD-HHMMSS` フォルダを使ってください。

## 困ったとき

### 「Port 5174 is already in use」と表示されて起動しない

旧版が使う画面用ポートが、起動済みのCat-Cutや他のアプリに使用されています。Nextcloudの最新zipから更新してください。2026年9月18日版以降は、空いているポートを自動で使い、二重起動時は既存画面へ戻ります。ポートを使用しているアプリを強制終了するコマンドは不要です。

更新前にCat-Cutが開いていれば、保存済みを確認して⌘Qで終了します。更新後はデスクトップの `Cat-Cut.command` から起動してください。ターミナルに表示されるnpmの更新通知は、このエラーの原因ではありません。
...
「Electron本体の起動確認 OK」の後に「アプリ依存(remotion)のインストールに失敗」と出た場合も、セットアップは未完了です。最新版を展開し直し、冒頭の配布ビルドとnpmキャッシュの保存先を確認して、最後の「インストール完了です」まで進めてください。
