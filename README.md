...
### 社員PCの起動安定化（2026-09-18）

- `npm run dev` はViteをNode APIで起動し、5174が使用中なら空きポートへ移動する。listenが成功した実際のURLをElectronへ渡す。
- Electron内のHTTP転送で画面のoriginは従来どおりに保ち、localStorageの確認済みチェックやフォント設定を引き継ぐ。他ポートの動画・音声配信はRangeヘッダーも保持して転送する。
- Electronは本体の初期化前にsingle instance lockを取得。二重起動は既存画面へ復帰し、余分なサーバーは終了する。
- アプリ終了・起動失敗・ターミナル終了で自身のサーバーを片付け、親の強制終了はIPC切断でElectronにも通知する。ポートを占有する他プロセスには触れない。
